...
public class BidInformation
{
    ...
    {
        HasInformation = records.Count != 0;
        minRecords = records.SelectMany(x => x).Where(x => x.Key.StartsWith("Min")).GroupBy(x => x.Key)
            .ToDictionary(g => g.Key, g => g.Select(x => int.Parse(x.Value)).Min());
        maxRecords = records.SelectMany(x => x).Where(x => x.Key.StartsWith("Max")).GroupBy(x => x.Key)
            .ToDictionary(g => g.Key, g => g.Select(x => int.Parse(x.Value)).Max());
        ...
        bool? GetHasProperty(string fieldName)
        {
            ...
            var p = recordsWithValue.Count == 0 ? null : (bool?)(int.Parse(recordsWithValue.FirstOrDefault().Value) == 1);
            return p;
        }
    }
    ...
    public string GetInformation()
    {
        ...
        string GetControlAsText()
        {
            ...
            foreach (var suit in Enum.GetValues(typeof(Suit)).Cast<Suit>().Except([Suit.NoTrump]))
